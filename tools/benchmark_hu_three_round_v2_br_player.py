...
EXPECTED_PURE_REPLAY_TERMINALS = 32 * 162
...
def main() -> None:
    ...
    if response.terminal_histories != EXPECTED_TERMINALS:
        raise SystemExit(
            f"V2 BR{args.player} terminal coverage mismatch: "
    ...
    if replay_terminals != EXPECTED_PURE_REPLAY_TERMINALS:
        raise SystemExit(
            f"V2 BR{args.player} pure replay work mismatch: "
            f"{replay_terminals} vs {EXPECTED_PURE_REPLAY_TERMINALS}"
        )
    if abs(response.value - replay) > 1e-10:
        raise SystemExit(
            f"V2 BR{args.player} pure replay mismatch: {response.value} vs {replay}"
        )

    print(
        f"v2 player={args.player} exact_br={response.value:.12f} "
    ...
    print(
        f"v2 player={args.player} independent_pure_replay={replay:.12f} "
        f"pure_replay_terminals={replay_terminals} replay_seconds={replay_seconds:.6f}"
    )
    print(f"HU THREE-ROUND V2 EXACT BR PLAYER {args.player}: PASS")
