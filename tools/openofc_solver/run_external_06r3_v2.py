...
"""Frozen EXT-06R3 V2 exact-exploitability cell runner."""
...
EXPERIMENT_ID = "EXT-06R3-V2-EXACT-EXPLOITABILITY-SOLVER-SELECTION"
...
def run(method: str, seed: int) -> dict:
    ...
    if not all(math.isfinite(x) for x in (br0.value, br1.value, nash_conv, exploitability)):
        raise AssertionError("06R3 exact tribunal produced a non-finite result")
    if exploitability < -1e-10:
        raise AssertionError("06R3 exploitability is negative beyond tolerance")

    payload = {
        "schema": "openofc-external-06r3-v2-cell-v1",
        ...
        "verdict": "PASS_06R3_CELL_NO_INDIVIDUAL_PROMOTION_AUTHORITY",
