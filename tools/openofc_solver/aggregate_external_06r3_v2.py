...
"""Aggregate authority for EXT-06R3 V2 exact-exploitability cells."""
...
EXPECTED_SCHEMA = "openofc-external-06r3-v2-cell-v1"
...
def _load(paths: list[Path]) -> dict[tuple[str, int], dict]:
    ...
    for path in paths:
        ...
        if payload.get("schema") != EXPECTED_SCHEMA:
            raise AssertionError(f"unexpected 06R3 schema in {path}")
        method = str(payload["method"])
        seed = int(payload["learner_seed"])
        key = (method, seed)
        if method not in EXPECTED_METHODS or seed not in EXPECTED_SEEDS:
            raise AssertionError(f"unexpected 06R3 cell {key}")
        if key in out:
            raise AssertionError(f"duplicate 06R3 cell {key}")
        ...
        if int(payload.get("real_routes_certified", -1)) != 0:
            raise AssertionError("06R3 cell exceeded authority boundary")
        if payload.get("verdict") != "PASS_06R3_CELL_NO_INDIVIDUAL_PROMOTION_AUTHORITY":
            raise AssertionError(f"06R3 cell did not pass: {key}")
        out[key] = payload

    expected = {(method, seed) for method in EXPECTED_METHODS for seed in EXPECTED_SEEDS}
    if set(out) != expected:
        raise AssertionError(f"incomplete 06R3 cell set: {sorted(out)}")
    return out
...
def run(paths: list[Path]) -> dict:
    ...
    if promote_mccfr and promote_isuct:
        raise AssertionError("06R3 contract allowed two promoted methods")

    if promote_mccfr:
        verdict = "PROMOTE_MCCFR_FROM_06R3"
    elif promote_isuct:
        verdict = "PROMOTE_ISUCT_FROM_06R3"
    else:
        verdict = "NO_SINGLE_METHOD_PROMOTION_06R3"

    payload = {
        "schema": "openofc-external-06r3-v2-aggregate-v1",
        "experiment_id": "EXT-06R3-V2-EXACT-EXPLOITABILITY-AGGREGATE",
        "authority": "FROZEN_06R3_GLOBAL_PROMOTION_AUTHORITY",
