...
Status: **DORMANT SHADOW RESEARCH / NOT CERTIFICATION**
...
For each frozen physical world, enumerate every positive-probability action branch under the fixed profile while carrying:
...
Normalize each set within the information state. Compare both against the Q1 resolver's uniform-compatible-state prior and against each other.

## Required metrics per information state

- actor and round;
- number of compatible concrete states;
- normalized full-reach weights;
- normalized counterfactual-reach weights;
- total variation: uniform vs full reach;
- total variation: uniform vs counterfactual reach;
- total variation: full vs counterfactual reach;
- maximum normalized weight;
- effective support size (`1 / sum(p^2)`).

The experiment summary must include maxima and distributional summaries by decision layer and by base algorithm.

## Interpretation

A large uniform-vs-reach TV means Q1's local completion may materially misweight hidden compatible states even though it no longer has an unseen-policy fallback. A small TV supports, but does not prove, the adequacy of the uniform finite-support prior for this reduced fixture.

Full-reach and counterfactual-reach differences are diagnostic. They must not be silently forced equal; a difference can reveal that the canonical information-state representation or perfect-recall assumptions deserve further audit.
