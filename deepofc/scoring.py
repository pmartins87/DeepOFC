...
def completed_board_ranks(board: PlayerBoard) -> tuple[HandRank, HandRank, HandRank]:
    ...
    top = rank_top([c for c in board.top if c is not None])
    middle = rank_five([c for c in board.middle if c is not None])
    bottom = rank_five([c for c in board.bottom if c is not None])
...
def is_foul(board: PlayerBoard, *, equality_allowed: bool) -> bool:
    ...
    R1 currently contains a real source conflict: supplied in-client rules say
    stronger-than-or-equal while the public KKPoker page reads as strict. This
    function intentionally has no default so callers cannot silently choose.
    """
